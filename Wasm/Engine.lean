...
mutual
  ...
  partial def getSO (locals : List (Option String × Option StackEntry))
    ...
    match getter with
    | Get'.from_stack => match stack with
      | [] => Except.error EngineErrors.not_enough_stuff_on_stack
      | s :: rest => Except.ok (rest, s)
    ...
    | .by_name n => match findLocal locals n.name with
      | .none => .error $
        EngineErrors.local_with_given_name_missing n.name
      | .some l => .ok (stack, l)
    | .by_index i => match locals.get? i.index with
      | .some (_, .some se)  => .ok (stack, se)
      | _ => .error $ EngineErrors.local_with_given_id_missing i.index
    | .i_const i => .ok $ (stack, StackEntry.num $ NumUniT.i i)
    | .f_const f => .ok $ (stack, StackEntry.num $ NumUniT.f f)
...
def runDo (_s : Store m)
  ...
  let bite acc x :=
    ...
    | .ok cont => match cont.1.es with
      | [] => Except.error EngineErrors.not_enough_stuff_on_stack -- TODO: Better errors lol lol
      | y :: rest => if paramTypecheck x y then
        Except.ok (Stack.mk rest, y :: cont.2)
      else
        Except.error .param_type_incompatible
  let pσ ← f.params.foldl bite $ Except.ok (σ, [])
  let locals : List (Option String × Option StackEntry) := (f.params ++ f.locals).map $
    fun l =>
      match l with
      | .name ll => (Option.some ll.name, pσ.2.get? ll.index)
      | .index ll => (.none, pσ.2.get? ll.index)
  let go (oσ : Except EngineErrors Stack) (x : Operation) : Except EngineErrors Stack := do
    let stack ← oσ
...
def run (s : Store m) (fid : Nat) (σ : Stack) : Except EngineErrors Stack :=
  let of := s.func.get? fid
  match of with
  | .none => Except.error .function_not_found
  | .some f => Except.mapError (fun x => x) $ runDo s f σ
